...
export default class Point {
  ...
  subdivide(point: Point, count: number, checkPoint: (point: Point) => Point) {
  ...
  segment(point: Point, percent: number) {
  ...
  midpoint(point: Point, location: number) {
    return this.segment(point, 0.5)
  }

  project(radius: number, percent: number = 1.0) {
  ...
  findCommonFace(other: Point, notThisFace: Face) {
